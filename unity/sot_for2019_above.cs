...
public static class BuildiOS
{
    ...
    public static void OnPostprocessBuild(BuildTarget buildTarget, string path)
    {
        ...
        //只对UnityFramework这个Target进行热更注入和编译补丁
        string UnityFramworkTarget = proj.TargetGuidByName("UnityFramework");
        ...
        if(bIsWebsiteVersion)
            proj.AddBuildProperty (UnityFramworkTarget, "OTHER_LDFLAGS", "-sotmodule UnityFramework /Users/sotsdk-1.0/libs/libsot_web.a -sotsaved $(SRCROOT)/sotsaved/$(CONFIGURATION)/$(CURRENT_ARCH) -sotconfig $(SRCROOT)/sotconfig.sh");
        else
            proj.AddBuildProperty (UnityFramworkTarget, "OTHER_LDFLAGS", "-sotmodule UnityFramework /Users/sotsdk-1.0/libs/libsot_free.a -sotsaved $(SRCROOT)/sotsaved/$(CONFIGURATION)/$(CURRENT_ARCH) -sotconfig $(SRCROOT)/sotconfig.sh");

        // 对il2cpp生成的C++文件进行热更注入
        proj.AddBuildProperty (UnityFramworkTarget, "OTHER_CFLAGS", "-sotmodule UnityFramework -sotconfig $(SRCROOT)/sotconfig.sh");
        //禁用BITCODE
        proj.SetBuildProperty(UnityFramworkTarget, "ENABLE_BITCODE", "false");
        //增加c++库和libz压缩库，用来解压补丁
        AddLibToProject(proj, UnityFramworkTarget,"libc++.tbd");
        AddLibToProject(proj, UnityFramworkTarget,"libz.tbd");

        
        string main_target = proj.GetUnityMainTargetGuid();
        ...
        //添加SDK头文件
        Preprocessor.WriteBelowCode("#include \"UnityInterface.h\"","#import \"/Users/sotsdk-1.0/libs/SotWebService.h\"");
        //添加调用代码来加载补丁
        if(bIsWebsiteVersion)
        {
            //去SOT官网注册(https://www.sotvm.com) 后，创建APP->创建版本，就能得到唯一的版本VersionKey，把下面12345678替换成正式的VersionKey即可
            string VersionKey = @"12345678";
            Preprocessor.WriteBeforeCode("[self frameworkWarmup: argc argv: argv];", GetWebSDKCallCode(VersionKey));
        }
        else
        {
            //免费版能从Bundle中加载补丁文件，可以用于本地测试SOT功能把
            Preprocessor.WriteBeforeCode("[self frameworkWarmup: argc argv: argv];", "[SotWebService ApplyBundleShip];");
        }
